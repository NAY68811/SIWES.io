...
export default function SupervisorStudents() {
  ...
  return (
    <div className="space-y-6" data-testid="supervisor-students">
      <h1 className="text-3xl font-extrabold tracking-tight">My Students</h1>
      {students.length === 0 && (
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {students.map(s => (
          <div key={s.id} className="rounded-xl border border-border bg-card p-5" data-testid={`student-card-${s.id}`}>
            ...
            <div className="mt-3 text-sm text-muted-foreground">{s.email}</div>
            {s.company ? (
              <div className="mt-4 border-t border-border pt-3">
                <div className="text-xs uppercase tracking-widest text-primary">Company</div>
